...
translation_source = [TsFile("Ribbon.ts", "FreeCAD_Ribbon.ts")]
...
wb_location = WBLocation("Ribbon", ".")  # to match filename
...
class CrowdinUpdater:
    ...
    def _get_source_files_info(self) -> dict:
        ...
        return {
            f["data"]["path"].strip("/"): str(f["data"]["id"]) for f in files
        }
    ...
    def _upload_translation_file(self, ts_file: TsFile, files_info: dict):
        ...
        locale = translation_filename.split("_")[1].split(".")[
            0
        ]  # Extracts 'es-ES'
        ...
        if src_filename in files_info:
            ...
            print(
                f"{translation_filename} was not updated because is not on the list"
            )
    ...
    def build(self):
        self._make_project_api_req(
            "/translations/builds", data={}, method="POST"
        )
...
def load_token() -> None | str:
    ...
    if not os.path.exists(config_file):
        config_file = (
            os.path.expanduser("~") + os.sep + ".crowdin-freecadaddons"
        )
...
def applyTranslations():
    ...
    if not os.path.exists(os.path.join(tempfolder, wb_location.module_name)):
        ...
        for short_locale, full_locale in supported_locales.items():
            ...
            oldname = f"FreeCAD_{wb_location.module_name}_{full_locale}.ts"
            newname = f"FreeCAD_{wb_location.module_name}_{short_locale}.ts"
            old_file = os.path.join(
                tempfolder, wb_location.module_name, oldname
            )
...
if __name__ == "__main__":
    ...
    if command == "status":
        ...
        locale_source = []
        for arg in args[1:]:
            locale_source.append(TsFile(f"Ribbon_{arg}.ts", f"Ribbon_{arg}.ts"))
